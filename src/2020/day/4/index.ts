...
export interface WellFormedRawPassport {
  byr: string;   // (Birth Year)
  iyr: string;   // (Issue Year)
  eyr: string;   // (Expiration Year)
  hgt: string;   // (Height)
...
function parseLine(l: string): RawPassport {
  const tokens = l.split(' ')
                     ...
                     }) as unknown as Array<[keyof RawPassport, string]>;

  return tokens.reduce<RawPassport>(
      (passport: RawPassport, [k, v]: [keyof RawPassport, string]) => {
        return !v ? passport : {...passport, [k]: v}
      },
      {});
}

async function parseInput(): Promise<RawPassport[]> {
  let passports: Array<RawPassport> = [{}];
  for await (const line of lines(
      '/Users/landrito/Code/advent-of-code/src/2020/day/4/input.txt')) {
    switch (line) {
      case '':
        passports = [{}, ...passports];
        break;
      default:
        const [head, ...tail] = passports;
        passports = [{...head, ...parseLine(line)}, ...tail]
    }
  }
  return Promise.resolve(passports);
}

function isWellFormed(p: RawPassport) {
  return !!p.byr && !!p.iyr && !!p.ecl && !!p.hcl && !!p.pid && !!p.hgt &&
      !!p.eyr;
}

const VALID_EYE_COLORS = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'];
const HEGIHT_MATCHER = /^(\d+)(in|cm)$/;
const YEAR_MATCHER = /^\d{4}$/;

function validatePassport(p: WellFormedRawPassport) {
  let valid = true;
  const byr = parseInt(p.byr);
  const iyr = parseInt(p.iyr);
  const eyr = parseInt(p.eyr);

  valid = [p.byr, p.iyr, p.eyr].reduce(
      (valid, str) => valid && YEAR_MATCHER.test(str), valid as boolean);
  valid = valid && 1920 <= byr && byr <= 2002;
  valid = valid && 2010 <= iyr && iyr <= 2020;
  valid = valid && 2020 <= eyr && eyr <= 2030;
  valid = valid && HEGIHT_MATCHER.test(p.hgt);
  valid = valid && /^#([a-fA-F0-9]{6})$/.test(p.hcl);
  valid = valid && VALID_EYE_COLORS.includes(p.ecl);
  valid = valid && /^0*(\d{9})$/.test(p.pid);
  if (valid) {
    const tags = p.hgt.match(HEGIHT_MATCHER) as unknown as RegExpMatchArray;
    const magnitude = parseInt(tags[1]);
    const unit = tags[2];

    switch (unit) {
      case 'cm':
        valid = valid && 150 <= magnitude && magnitude <= 193;
        break;
      case 'in':
        valid = valid && 59 <= magnitude && magnitude <= 76;
        break;
      default:
        console.assert(`Unexpected height unit: ${unit}`);
    }
  }
  return valid;
}
...
async function main() {
  const rawPassports = parseInput();
  console.log('Part 1');
  const wellFormedPassports =
      (await rawPassports)
          .filter(isWellFormed)
          .map(p => p as unknown as WellFormedRawPassport);
  console.log(wellFormedPassports.length);

  console.log('Part 2');
  const validPassports = wellFormedPassports.filter(validatePassport);
